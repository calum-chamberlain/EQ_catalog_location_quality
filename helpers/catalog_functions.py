...
def get_min_dist_degrees(event: Event, inventory: Inventory) -> float:
    ...
    for arrival in origin.arrivals:
        ...
        dist = haversine(
            ...
            lon2=pick_coords.longitude, lat2=pick_coords.latitude)
        if dist < min_dist:
            min_dist = dist
    return min_dist
...
def lookup_station_coords(
    ...
    channel_locations = {
        ...
        for n in selected for s in n for c in s}
...
def min_dist_calculator(
    ...
    min_dist_m, _, _ = gps2dist_azimuth(
        ...
        lat2=chan_coords.latitude, lon2=chan_coords.longitude)
...
def plot_catalog(cat, lats, lons, depths, mags, poly, bblons, bblats, splons, splats, region):
...
def plot_quality_criteria_scores(fixed_inv, cat_poly, s_picks, min_dist_bi, ps, min_picks, min_az_bi, region):
    ...
    counts=[fixed_inv.count(1) / len(cat_poly) * 100, 
